...
async def send_actual_config_info(update, context):
    ...
    warehouse_names = [ware_house_dict.get(wh_id, wh_id) for wh_id in desired_warehouses]
...
{langFile['actualWareHouse']}: {"\n" + ("\n".join(warehouse_names) if desired_warehouses else langFile['allWareHouse'])}
"""
